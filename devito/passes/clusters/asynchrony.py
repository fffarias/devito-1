...
from devito.ir import (Forward, GuardBound, GuardBoundNext, Queue, Vector, WaitLock,
                       WithLock, FetchUpdate, FetchPrefetch, PrefetchUpdate,
                       WaitPrefetch, Delete, normalize_syncs, SEQUENTIAL)
    ...
    def callback(self, clusters, prefix):
        ...
        for c0 in clusters:
            ...
            for f in protected:
                ...
                # The locked data space, which represents the data items in `f`
                # that are guaranteed not to be written while a lock is acquired
                dspace = c0.ispace.project(lambda i: i is not d)

                tasks[c0].extend(WithLock(lock[i], dspace) for i in indices)
